# DB 스키마 관리 가이드 (tutoring.picklass.com)

## 배경
자매 프로젝트 `picklass-backoffice`에서 Prisma 마이그레이션(`prisma migrate`, `prisma db push`) 실행 중 **데이터 롤백/삭제 이슈**가 발생하여 2026-05-22에 폐기되었다. tutoring도 동일 정책을 적용한다.

앞으로 DB 스키마 변경은 **DBMS에서 직접 DDL을 실행**하고, Prisma는 타입/쿼리 빌더 용도로만 사용한다.

> tutoring은 speaking.picklass.com과 Supabase를 **공유**한다. 공통 테이블(`users`, `code_*`) 변경은 speaking 팀과 사전 정렬 필수.
...
| `npx prisma migrate ...` 형태 | 위 명령의 우회 호출 |

`apps/api/package.json`에 위 명령들은 등록하지 않는다.

## 허용 명령어

| 명령 | 용도 |
|---|---|
| `pnpm --filter @tutoring/api prisma:pull` | DB → schema.prisma 동기화 (introspection, **DB 변경 안 함**) |
| `pnpm --filter @tutoring/api prisma:generate` | Prisma Client 코드 재생성 |
| `pnpm --filter @tutoring/api build` | Prisma generate 포함 빌드 |
...
**공유 DB 주의**: Supabase가 speaking.picklass.com과 공유되므로 공통 테이블(`users`, `code_*`) 변경 시 양쪽 영향 검토 필수.
...
```bash
pnpm --filter @tutoring/api build
```
- `build` 스크립트가 `prisma generate`를 포함
- API 프로세스 재시작
