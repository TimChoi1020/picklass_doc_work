...
|---|---|---|---|---|---|
| BO-F-010 | 조직 목록 조회·필터 | BO-S-010 | P-004, P-005 | E-004, E-005 | ✅ |
| BO-F-011 | 조직 등록(플랜 자동채움) | BO-S-011 | P-010, P-013 | E-001, E-002 | ✅ |
| BO-F-012 | 조직 수정 | BO-S-013 | P-010 | E-001 | ✅ |
| BO-F-013 | 기관 수업설정 편집 | BO-S-013 | P-023 | E-001 | ✅ |
| BO-F-020 | 사용자 목록 조회·필터 | BO-S-020 | P-004, P-011 | E-005 | ⚠️ Scope UI만 |
| BO-F-021 | 사용자 등록 | BO-S-021 | P-010 | E-002 | ✅ |
| BO-F-022 | 사용자 수정 | BO-S-022 | — | E-001 | ✅ |
| BO-F-023 | 자동 로그인(handoff) | BO-S-020 | P-012 | E-007 | ✅ |
| BO-F-024 | 아이디 일괄생성 | BO-S-023 | P-010 | E-003 | ✅ |
| BO-F-030 | 과정 목록 조회·필터 | BO-S-031 | P-004, P-032, P-033 | — | ✅ |
| BO-F-031 | 과정 상세 조회 | BO-S-032 | P-006 | E-006 | ✅ |
| BO-F-032 | 과정 목록 내보내기 | BO-S-031 | — | — | ✅ |
| BO-F-033 | 액세스코드 목록·필터 | BO-S-033 | P-004, P-020 | — | ✅ |
| BO-F-034 | 액세스코드 생성 | BO-S-034 | P-010, P-020 | E-001, E-003 | ✅ |
| BO-F-035 | 액세스코드 상태전환 | BO-S-033 | P-020 | E-008 | ✅ |
| BO-F-036 | 과정 카테고리 관리(L1/L2) | BO-S-035 | P-030 | E-011 | ⚠️ 일부 미연동 |
| BO-F-037 | 과정 카테고리 배정 | BO-S-035 | P-031 | E-011 | ⚠️ bulk 미구현 |
| BO-F-040 | 기관 학습 현황(드릴다운) | BO-S-041 | P-004, P-005 | E-004 | ✅ |
| BO-F-041 | 출석률 | BO-S-043 | P-004 | — | ✅ |
| BO-F-042 | B2B 학습데이터 | BO-S-044 | P-004 | — | ✅ |
| BO-F-043 | 플랫폼 현황 | BO-S-040 | P-002 | — | ✅ |
| BO-F-050 | 코드관리 | BO-S-050 | — | E-010 | ✅ |
| BO-F-051 | 수업모듈 관리 | BO-S-051 | — | — | ✅ |
| BO-F-052 | 시스템관리자 계정 | BO-S-054 | P-002 | — | ✅ |
| BO-F-053 | API키관리 | BO-S-055 | — | — | ⚠️ 미연동 |
| BO-F-060 | API 연동 가이드 | BO-S-060 | P-007 | — | ⚠️ Step1 |
| BO-F-061 | 내 연동 설정 | BO-S-061 | P-002, P-007 | — | 🔲 Step2 |
| BO-F-062 | 연동 테스트 | BO-S-062 | P-002 | — | 🔲 Step3 |
...
- **동작**: 4섹션 — ①가입정보(아이디이메일[중복확인 필수]·임시비번·기관명·기관유형[개인학원/프랜차이즈/어학원/공교육/기업교육]·담당자·회원상태) ②부가정보(지점수·운영형태[직영/가맹]·수강생규모) ③라이선스·요금제 ④계약정보(계약상태[협의중/계약완료/계약중/만료/해지]·시작/종료일·자동갱신). 요금제 선택 시 **DEFAULT_PLANS**(Starter/Pro/Enterprise) 값 자동채움 후 편집 가능(P-013). `POST /api/institutions`.
- **AC**:
  - ☐ Given 요금제 선택, Then monthlyPrice·baseStudents·pricePerStudent·maxAdminAccounts 등이 자동 채워지고 이후 수정 가능하다
  - ☐ Given 이메일 중복확인 미완료, When 등록, Then 차단·"이메일 중복 확인을 완료해주세요"(E-002)
  - ☐ Given 계약 시작일 ≥ 종료일, Then 검증 실패(E-001)
- **정책**: P-010, P-013 · **예외**: E-001, E-002

### BO-F-013 · 기관 수업설정 편집
- **화면**: BO-S-013 (institution type 상세 `[수업설정]` 탭)
- **User Story**: 기관 관리자로서, 소속 학생의 학습 정책을 통제하기 위해, 수업·FRT 정책을 설정하고 싶다.
- **동작**: `GET/PUT /institutions/:id/settings`. [과정운영] maxTotalClassMinutes(무제한/5~50분)·defaultCompletionSentences·defaultCompletionLessonRate(0~100%)·defaultCompletionMinutes·freeLearningEnabled. [FRT모듈] maxFrtModuleMinutes(무제한/5~30분)·allowedMicModes(PTT/Hybrid/Always on, **최소 1개 강제**)·micModeUserChange. → **Studio 과정생성 speaking_options 초기값으로 전달**(저장 후 독립, P-023).
- **AC**:
...
- **AC**:
  - ☐ Given 이메일 형식 위반, Then 검증 실패(E-001)
  - ☐ Given 중복 이메일, Then "이미 사용 중인 이메일입니다"(E-002)
  - ☐ Then 신규 계정은 즉시 active로 생성된다

### BO-F-023 · 자동 로그인 (handoff)
- **화면**: BO-S-020 (목록 행 액션)
- **User Story**: 관리자로서, 강사/학생 화면을 직접 확인하기 위해, 해당 계정으로 대신 로그인하고 싶다.
- **동작**: `POST /handoff` → redirectUrl 새 탭. teacher→Studio, student→Tutoring. **핸드오프 JWT 만료 60초 단명**, JWT_SECRET 3시스템 공유(P-012).
- **AC**:
  - ☐ Given 강사 행, When 자동 로그인, Then Studio가 새 탭에서 로그인된 상태로 열린다
  - ☐ Given 핸드오프 토큰 60초 경과, Then 재사용 불가
  - ☐ Given 실패, Then "자동 로그인에 실패했습니다"(E-007)
- **정책**: P-012 · **예외**: E-007
...
- **AC**:
  - ☐ Given idCount>1000, Then "최대 1,000개까지 생성 가능합니다"(E-003)
...
- **동작**: 컬럼 과정명·레벨·장르·총레슨수·수강인원·배포횟수·상태(in_use/archived)·소속기관(+타입 뱃지). 필터 상태·레벨·장르·검색 + 조직 cascading. **수강인원/배포횟수는 응답시점 access_codes 실시간 집계**(비정규화 컬럼 폐기, `resolveAccessCodeCounts` 배치, P-033). `GET /courses`. 생성·편집은 **Studio 전담**(P-032).
- **AC**:
  - ☐ Then 배포횟수=courseId 연결 코드 전체수, 수강인원=그중 statusCode='active'
  - ☐ Given 관리자 역할, Then 생성/편집/삭제 액션은 노출되지 않는다(읽기전용)
- **정책**: P-004, P-032, P-033
...
- **AC**:
  - ☐ Given 학생 유형, When 과정 미선택, Then "학생 선택 시 제공할 과정을 선택해주세요"(E-001)
  - ☐ Given expiryDate 과거, Then "등록 만료일은 오늘 이후로 설정해주세요"
  - ☐ Given 생성, Then 코드 N건이 inactive로 생성된다
- **정책**: P-010, P-020 · **예외**: E-001, E-003

### BO-F-035 · 액세스코드 상태전환
- **화면**: BO-S-033
- **동작**: inactive→활성화, active→비활성화, completed/expired→버튼 없음(최종). `activate()`는 사전설정 usageStart/End 우선, 없으면 등록시점 자동계산(P-020).
- **AC**:
...
- **동작**: L1(대)/L2(중) 2-depth, `course_categories`(partner_id·depth CHECK(1,2)·parent_id·series 등). `GET/POST/PATCH/DELETE /course-categories`·`/reorder`. 배정: 과정은 **L2에만 배정**(L1 단독 금지, P-031), **동일 파트너 카테고리만**. 삭제: L2 CASCADE, courses SET NULL.
- **AC**:
  - ☐ Given L1에만 배정 시도, Then BadRequest(E-011)
  - ☐ Given 교차 파트너 카테고리 배정, Then 가드로 차단(E-011)
  - ⚠️ `POST /courses/bulk-assign-category` 미구현
- **정책**: P-030, P-031 · **예외**: E-011
...
- **AC**:
  ...
  - ☐ Given academy_admin, Then 이 메뉴에 접근할 수 없다(ORG_ABOVE, E-004)
...
- **BO-F-050 코드관리**(BO-S-050/052): 공통코드·상수 관리(USER_STATUSES·ACCESS_CODE_STATUSES·ACCESS_CODE_DURATIONS·LEVEL_SYSTEM 18단계). 저장 시 트랜잭션 타임아웃 주의(E-010).
...
- **BO-F-061 내 연동 설정**(BO-S-061): `GET/POST/PUT /b2b/api-configs`·`PATCH /:id/status`. partner_admin은 조회전용(등록·수정 SYS 전용, P-002). 🔲 Phase F-3.
