...
|---|---|---|---|---|---|
| ST-F-001 | 로그인 | ST-S-050 | ST-P-024 | ST-E-005 | ✅ |
| ST-F-002 | 회원가입 | ST-S-051 | — | ST-E-001 | ✅ |
| ST-F-010 | 지문 목록 조회·필터 | ST-S-010 | ST-P-023 | — | ✅ |
| ST-F-011 | AI 지문 생성 | ST-S-013 | ST-P-025 | ST-E-001 | ✅ |
| ST-F-012 | 지문 저장 | ST-S-013 | — | ST-E-012 | ✅ |
| ST-F-013 | 지문 난이도 분석 조회 | ST-S-014 | — | — | ⚠️ 하드코딩 |
| ST-F-015 | 수업 준비(KPI 시퀀싱) | ST-S-011 | ST-P-020 | ST-E-004 | ✅ |
| ST-F-016 | 수업 진행 | ST-S-012 | — | — | ✅ |
| ST-F-020 | 과정 목록 조회·필터 | ST-S-020 | ST-P-003, ST-P-031 | — | ✅ |
| ST-F-021 | 과정 생성 마법사 | ST-S-022 | ST-P-011~012, ST-P-040~044, ST-P-052 | ST-E-006, ST-E-010, ST-E-012 | ✅ |
| ST-F-022 | 과정 액세스코드 생성(행 액션) | ST-S-020 | ST-P-030 | ST-E-007 | ✅ |
| ST-F-023 | 과정 편집(정보·운영·카테고리) | ST-S-023 | ST-P-041~043, ST-P-051 | ST-E-010 | ✅ |
| ST-F-024 | 레슨 순서 이동 | ST-S-021 | — | — | ✅ |
| ST-F-025 | 레슨 모듈 수정 | ST-S-021 | ST-P-031 | ST-E-009 | ✅ |
| ST-F-026 | 레슨 추가 | ST-S-021 | — | ST-E-011 | ✅ |
| ST-F-027 | 레슨 과정에서 제외 | ST-S-021 | ST-P-031 | ST-E-009 | ✅ |
| ST-F-030 | 학생 목록 조회·필터 | ST-S-030 | ST-P-001 | — | ⚠️ mock |
| ST-F-031 | 학생 단건 등록 | ST-S-030 | ST-P-013 | ST-E-002 | ⚠️ mock |
| ST-F-032 | 학생 정보 수정 | ST-S-030 | ST-P-032 | ST-E-001 | ⚠️ mock |
| ST-F-033 | 학생 일괄 등록(+코드) | ST-S-030 | ST-P-010, ST-P-013 | ST-E-002, ST-E-003 | ⚠️ mock |
| ST-F-034 | 액세스코드 일괄 생성 | ST-S-031 | ST-P-010, ST-P-014, ST-P-030 | ST-E-003, ST-E-007 | ✅ |
| ST-F-035 | 액세스코드 상태 전환 | ST-S-031 | ST-P-030 | ST-E-008 | ✅ |
| ST-F-040 | 리포트 목록 조회·필터 | ST-S-040 | ST-P-003, ST-P-015 | — | ✅ |
...
- **인수조건(AC)**:
  ...
  - ☐ Given 생성 실패(API 오류), Then ST-E-001 방식으로 오류 문구를 표시하고 모달을 유지한다
- **연관 정책**: ST-P-025(지문 콘텐츠 안전성 검증) · **연관 예외**: ST-E-001
...
- **AC**:
  ...
  - ☐ Given 저장 경로 불일치 버그(§7 ST-E-012), Then 저장 후 목록에 즉시 표시되어야 한다(현재 알려진 결함)
...
- **AC**:
  ...
  - ☐ Given 시퀀싱 실패, Then 모듈 선택·"수업 들어가기"가 차단된다(§7 ST-E-004)
  - ☐ Given 모듈 ≥1 선택 + 시퀀싱 성공, Then "수업 들어가기"가 활성화된다
- **연관 정책**: ST-P-020(시간 프리셋) · **연관 예외**: ST-E-004
...
- **AC**:
  - ☐ Given Step 0-A, Then 유형에 따라 이후 모듈 선택 범위가 결정된다(ST-P-052)
  - ☐ Given KPI 미선택, When "다음", Then `course_goal='일반 영어 학습'` 폴백으로 400을 방지한다(ST-E-006)
  - ☐ Given 운영 설정이 기관 한도를 초과, Then 해당 프리셋이 disabled이며 서버가 400으로 재검증한다(ST-P-041, ST-P-043, ST-E-010)
  - ☐ Given 회차 일부 지문 생성 실패, Then 해당 회차만 `text_id=null`로 저장되고 나머지는 정상 저장된다(ST-E-012)
  - ☐ Then 통합(integrated) 과정은 회차 내 speaking 모듈이 항상 후순위로 배치된다(ST-P-052)
- **연관 정책**: ST-P-011~012, ST-P-040~044, ST-P-052 · **연관 예외**: ST-E-006, ST-E-010, ST-E-012
...
- **AC**:
  - ☐ Given L1 미선택, Then L2 Select는 비활성이다(ST-P-051)
  - ☐ Given L1 선택+L2 미선택, When 저장, Then `l2_category_id=null`(미분류 하위)로 저장된다
  - ⚠️ Given 운영설정 한도 초과, Then UI는 막지만 **편집 PATCH는 서버 재검증이 미이식**됨(ST-P-043, 알려진 갭)
- **연관 정책**: ST-P-041~043, ST-P-051 · **연관 예외**: ST-E-010
...
- **ST-F-025 모듈 수정**: `MODULE_CATEGORIES` 체크박스(0개 허용). `PATCH .../lessons/:id/modules`. **in-use 레슨은 disabled**(ST-P-031, ST-E-009)
- **ST-F-026 레슨 추가**: Step1 텍스트 선택(필터·라디오) → Step2 모듈 선택(≥1) → source 레슨 **앞에** splice 삽입 후 round 재계산. 추가 레슨 status 항상 `not-used`
- **ST-F-027 과정에서 제외**: `DELETE .../lessons/:id`. **in-use 레슨은 disabled**(ST-P-031, ST-E-009)
- **AC**:
  - ☐ Given 레슨 추가 Step1, When 텍스트 미선택, Then "다음" 불가(toast)
  - ☐ Given Step2, When 모듈 미선택, Then "추가" disabled(ST-E-011)
...
- **AC**:
  - ☐ Given 아이디 입력, When 중복확인 미통과/미실행, Then 등록 버튼이 차단된다(ST-E-002)
  - ☐ Given 형식 `{4}{3}@{4}.pick` 위반, Then 등록 불가(ST-P-013)
  - ☐ Then 신규 학생 상태는 `active`로 생성된다(ST-P-032)
...
- **AC**:
  - ☐ Given 개수 <1 또는 >1000, Then ST-E-003 오류(ST-P-010)
...
- **AC**:
  ...
  - ☐ Given 개수 범위 위반, Then ST-E-003 표시·생성 안 함
  - ☐ Given 코드 6자리 영대문자+숫자, Then 전역 유니크로 발급된다(ST-P-014)
- **연관 정책**: ST-P-010, ST-P-014, ST-P-030 · **연관 예외**: ST-E-003, ST-E-007
...
- **AC**:
  ...
  - ☐ Given completed/expired 코드, Then 상태 전환 버튼이 없다(ST-E-008)
- **연관 정책**: ST-P-030 · **연관 예외**: ST-E-008
...
- **동작**: `/login`·`/signup`. 세션 `JWT_EXPIRES_IN=7d`, IDLE 7200s, keepalive 4분 주기 `/auth/me`(ST-P-024).
- **AC**:
  - ☐ Given 세션 만료(401), Then 즉시 로그아웃이 아니라 **SessionExpiredDialog**로 안내한다(ST-E-005)
