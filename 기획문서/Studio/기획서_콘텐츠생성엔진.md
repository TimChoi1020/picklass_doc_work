...
interface GenerationRequest {
  ...
  targetLevel: number;              // 목표 레벨 (1~18 세분 레벨; CEFR 7밴드에 매핑)
...
| 상황 | 모델 선택 |
|---|---|
| 단일 고품질 지문 | Gemini 2.5 Flash 홀리스틱 생성 |
| 대량 배치(주제 확장) | Google GenAI 병렬 생성 |
| 짧은 재생성(부분 수정) | Gemini 2.5 Flash 스트리밍 + 부분 교정 프롬프트 |
...
#### 9.3.6 원장 형식 템플릿 (Director Format Template) *(v1.4 신설 — 제안서 v0.3 §6 차별점 ③ 반영)*

> 📋 **기획 — 엔진 핵심 역량**. 출처: `제안서 v0.3` §6 차별점 ③. 본 엔진의 차별점 축은 **① 학원 범위 종속(*무슨 내용*)** 과 **② 형식 자유도(*어떤 형태*)** 두 가지이며, 본 절은 후자(②)를 담당한다.

**핵심 명제** — AI가 초안을 대신 만들어주는 데 그치지 않고, **원장이 학생을 가르치는 그 방식·형식 그대로** 자료를 생성한다. 진짜 차별점은 "AI 생성물을 원장이 검수하느냐"가 아니라, **원장마다 다른 자기만의 자료 형식·구성을 자유롭게 구현**하는 유연함이다.

**기능 (형식 템플릿)** — 원장이 **문항 형태·구성·출력 레이아웃을 지정·저장**하면, 같은 범위·같은 내용을 **그 형태로 반복 생성**한다.

- 형식 템플릿은 `GenerationRequest`(§9.2)와 직교하는 **출력 형식 스펙**으로 저장되어, 범위·내용 입력이 바뀌어도 원장이 지정한 형태로 재적용된다.
- 검수·AI 개입강도(최소/보통/최대)·톤은 이 유연성을 **받치는 신뢰 장치(부속물)**일 뿐, 차별점의 본질이 아니다.

**예시 (단어 퀴즈 3형태)** — 같은 단어 범위, 원장이 선택한 형식대로 반복 생성:

| # | 형식 | 제시 | 학생 작성 |
|---|---|---|---|
| ① | 뜻→단어 | 뜻 30개 제시 | 대응 영단어 작성 |
| ② | 음성→철자+뜻 | 단어 음성 제시 | 철자 + 뜻 작성 |
| ③ | 표 블랭크 채우기 | 단어·뜻 표에서 한쪽(단어 또는 뜻)을 블랭크로 비움 | 비워진 칸 채우기 |

### 9.4 분석 파이프라인

#### 9.4.1 CEFR 레벨 추정 — 8개 정량 지표 + LLM 하이브리드

정량 지표 8종과 LLM 정성 분석을 결합한 **하이브리드 레벨 추정 엔진**이 지문을 분석해 1–18 세분 레벨과 신뢰도 점수를 산출한다(코드 필드명 `cefrLevel`은 1~18 세분 레벨 스케일이며, 표준 CEFR는 7밴드 — [공통 06 진단평가](../00_공통/06_진단평가_PPD.md) 매핑 기준). 정량 지표로 포착하기 어려운 문맥·주제·배경지식 요소를 LLM 정성 분석이 보완한다.
...
5. 최종 레벨 1~18(세분 레벨, CEFR 7밴드 매핑) + 신뢰도(0~1) 출력
```
